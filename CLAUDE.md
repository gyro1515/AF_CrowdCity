...
## 1.1 Per-Area Changelog and Operational Contract

Split area documentation by **purpose**, not by language: **append-only change history** in one file, the **mutable operational contract** in another. Both are Korean. (An earlier version of this rule split them by language — an `en` "agent-facing" file plus a `ko` "human-facing" one. Withdrawn: the real distinction was purpose, and Korean costs nothing with either reader — Codex read this project's Korean docs accurately throughout. Do not re-derive the language split.)

- **Change history — `Docs/<Area>/CHANGELOG.md`.** Every commit that adds or changes a feature — perf, fix, and tooling included — adds that area's entry **in the same commit**. Pure docs/chore commits (typos, model-pin updates, transcript cleanup) are not logged. Newest entry first. It is **human-facing and detailed, not a summary**: logic, structure, why each decision was made, and rejected alternatives with the reason. Structure it for Notion (table of contents, `##`/`###` hierarchy, `<details>` folds). Every entry carries a **runnable `직접 확인하는 법`** line — the command or path that lets the reader verify that entry's headline claim themselves; write it in the project's primary shell and run it before committing it. Live example: `Docs/CrowdCity/CHANGELOG.md`.
- **Operational contract — that area's handoff doc** (`Docs/CrowdCity/SIM_OPT_HANDOFF.md` for CrowdCity). Holds current state / next step, standing invariants, verification gates, and traps. **This is the thing to read before starting work in the area.** Keep it current as state changes — it moves whenever the working state moves, including when no commit is produced (e.g. a new measurement reopens a settled next-step verdict).
- **Facts live in exactly ONE place: history in the changelog, contract in the handoff doc.** The changelog may carry a coarse orientation block, but invariants, gates, thresholds, hashes, and `file:line` citations belong in the handoff doc only — duplicating a volatile list guarantees one copy goes stale silently. Do not copy either into `CLAUDE.md`, and do not create a third home.
