...
This `AGENTS.md` applies to the repository root and everything below it. [`UnityArchitectureGuide/TEAM_ARCHITECTURE_GUIDE.md`](UnityArchitectureGuide/TEAM_ARCHITECTURE_GUIDE.md) is **general Unity architecture reference material**, not an authority for this project: it holds no project facts — no `Assets/@Project` paths, no invariants, no gate values — and its examples come from other games — read it for background on a pattern this file only summarizes, and never let it override a rule here. This project's own authorities are the **code**, `Docs/WORK_STATE.md`, and `Docs/PROJECT_MAP.md` (§1.1). If a more specific instruction file applies to a nested scope, merge it with this document, and surface any conflict instead of hiding it.
...
- **Session start — check `Docs/WORK_STATE.md`'s freshness stamp before trusting it.** Compare the hash at the top of that file against `git rev-parse --short HEAD`: **same** → proceed; **different** → read `git log --oneline --name-status <recorded>..HEAD` and let that range be the verdict: suspect only where those commits moved, so re-read the code there. `--name-status`, not bare `--oneline`: subjects triage but cannot establish semantic impact. Whoever updates `WORK_STATE.md` updates that hash in the same commit; the stamp names the commit the doc was **verified against**, necessarily the recording commit's parent — a commit cannot contain its own hash (`--amend` only yields a third), so do not try to amend one in. Do **not** re-add a pathspec pre-filter (`-- Assets/` or narrower): by construction it can only ever *clear* knowledge-changing docs commits — `ee73897..2421d12 -- Assets/` is empty though `2421d12` added the harness-override warning to `WORK_STATE.md` itself. That is a structural blind spot, not an observed false clear. What gets ignored is an alarm demanding *expensive* action on a common trigger; the demand has shrunk from "distrust the document" to "read the commit range" — routine, like `git status`. The reason is what makes this stick: normally the user says what to record before a session ends, so this is **not** a substitute for that — it exists for when the machine dies mid-work with no wrap-up, leaving the next session a confidently-wrong document, worse than none because a doc gets trusted where absent code gets read.

## Cross-Verification: Codex ↔ Claude (Mandatory)

For every non-trivial code task, both the **work plan** (before implementation) and the **post-work verification** (after implementation) must be cross-reviewed by two independent agents, each acting as a **Unity senior game programmer** — one Codex, one Claude. Every reviewer is **read-only** and receives **identical inputs**: the same HEAD SHA, a frozen working tree, and the same requirements, Decision Log, and verification results — a review of a tree that moved under it says nothing about what ships. Each independently critiques the other's plan/result; iterate in rounds until they reach **explicit consensus**, which means **zero unresolved BLOCKER/MAJOR findings**. Do not conclude a phase (plan or verification) while a BLOCKER or MAJOR is still open: record it and run another round. Never paper over a deadlock — if one survives the rounds, escalate it to the user as a decision (when the user is unavailable, the rule below applies); a finding is closed by a recorded decision, never by proceeding past it. The main agent orchestrates the exchange and judges convergence; a single agent's approval never substitutes for the cross-review. Trivial edits are exempt (use judgment).
...
- **Codex** — model `gpt-5.6-sol`, effort `ultra`, speed `fast` (invoke: `codex exec -m gpt-5.6-sol -c model_reasoning_effort=ultra -c service_tier=fast --skip-git-repo-check --sandbox read-only "<prompt>" < /dev/null`; requires codex-cli ≥ 0.144.5). A review call is **`--sandbox read-only`** — that flag is what enforces the read-only reviewer above; reserve `--sandbox workspace-write` for a call that must write files, which a review never is (`--skip-git-repo-check` only waives the Git-repository precondition — it grants no write access). **Always redirect stdin `< /dev/null`** — otherwise `codex exec` blocks waiting on stdin and emits no output until the command times out — and run the line from the **Bash** tool, never PowerShell, where `<` is a reserved operator that kills the command before Codex starts. Run it **synchronously in the foreground** (a single blocking call); never background it or wrap it in a poll/Monitor loop (that reintroduces the stdin hang and stalls the agent).
- **Claude** — model `Opus 5`, effort `xhigh`.
- **Never substitute silently.** If a specified CLI, its authentication, or a specified model is unavailable, stop and ask the user rather than swapping in what is at hand — the cross-review's whole value is that the two reviewers are the two specified independent agents.
...
Three documents, split by **the question each answers** — not by language, not by area. **Language follows the reader:** the AI-facing docs (`CLAUDE.md` / `AGENTS.md`, `PROJECT_MAP.md`, `WORK_STATE.md`) are **English**; the human guide is **Korean**, deliberately and permanently, because its reader is human. A file's current language is never a counter-rule — where a file disagrees, the file is what changes, and a document carrying exact hashes, thresholds, or gate values converts in its own careful pass because a translation slip there silently voids a verification gate. English for the AI-facing docs is a **token-cost** choice, not a comprehension one: they are read at every session start and again by every subagent pointed at them, and Korean runs ~1.5–2× the tokens for equivalent content — both Claude and Codex have read this project's Korean docs accurately.
...
- **Rule 4 — cite by literal identifier.** In tracked docs, name commits, branches, and versions by their literal identifier (hash, branch name, version number) — never by a self- or time-relative phrase ("this commit", "the current branch", "the latest measurement", "next pass"). The text stays fixed while the referent moves, so such a reference silently retargets as history grows, and no freshness check can catch it.
