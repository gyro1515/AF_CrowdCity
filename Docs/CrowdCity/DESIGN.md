...
- Unity **6000.3.9f1**, URP 17.3.0, Linear. **Input System 1.18.0 ONLY**. No UniTask/DOTween/Cinemachine/Addressables/TMP-essentials. uGUI 2.0. No new packages.
...
```txt
1. Target feature owner: Crowd (CrowdRoot: crowds, neutrals, sim kernel, crowd events, AND the Human passive leaf it clones/drives — Human is Crowd-OWNED despite living in the Human folder for asset cohesion) / Game (GameSceneController, GameSession, GameplayRoot, InputRoot, CameraRoot, GameConfigSO, editor setup/validator) / Hud (HudRoot)
2. New file location: Assets/@Project/Crowd/{Core, Scripts, Contracts/Events, Tests/Editor}, Assets/@Project/Game/{Scripts, Contracts/Runtime, Editor}, Assets/@Project/Hud/Scripts, Assets/@Project/Human/Scripts/Human.cs (stub rewrite), Assets/@Project/Human/Materials + Animations (assets generated by editor setup)
3. State location: GameConfigSO asset = immutable balance/config + team material references; session state in GameSession + CrowdModel plain-C# runtime models; no SaveData; no Server (no economy)
4. Communication method: parent→child direct calls; child→parent C# events; sibling binding at GameplayRoot; static EventManager bus for exactly two crowd-domain facts (justification §3); QueryBus track unused
5. Affected files: §6 table (~26 new files); modified: Human.cs (stub rewrite), GameScene.unity + Human_Base.fbx importer + generated .controller/.mat/.asset (ALL via editor setup E1, which owns every editor-time mutation)
```
...
- **HUD**: runtime uGUI (legacy `Text` + `LegacyRuntime.ttf`, CanvasScaler, no EventSystem/no Buttons): timer top-center, 4-row leaderboard top-right, start hint, result overlay (+standings, "R or tap to restart"). World-space count labels above leaders: billboarded legacy `TextMesh`, updated only on count events.
...
```
GameScene
└─ GameSceneController (only new scene GO; serialized: GameConfigSO, humanTemplate(=GameArea/Human), mainCamera, cityRoot(=GameArea/City))
   │  Awake: validate refs, deactivate template. Start: assemble (below). Executes reload on request. NOTHING else.
   ...
   └─ GameplayRoot (child GO, MB) — creates + binds + DRIVES the ordered tick
        ...
        ├─ CrowdRoot  (MB, Crowd): owns kernel sim, CrowdModels, Human clones; publishes the two bus events (⟲⟲ world-space leader labels belong to HudRoot — ALL presentation in one owner; CrowdRoot stays sim-only)
        ├─ CameraRoot (MB, Game) : drives Main Camera; subscribes both bus events
        └─ HudRoot    (MB, Hud)  : builds uGUI; subscribes both bus events; reads IGameSessionReadOnly
```

⟲ **Pinned init order** (fixes subscribe-before-first-publish): controller.Start → `new GameSession(config)` → create GameplayRoot → `gameplayRoot.Initialize(...)` which: creates all four roots → calls every root's `Initialize(...)` (ALL bus subscriptions + C# event bindings registered here) → `session.Initialize()` (its subscriptions) → `crowdRoot.SpawnInitial()` (first coalesced count publish — everyone already listening) → session enters Ready.
...
⟲ **Re-entrancy contract**: CrowdRoot publishes all events only AFTER all tick mutation completes (last phase of SimTick). Session state changes triggered by those events (e.g. elimination → Finished → `StateChanged`) reach CrowdRoot as `OnMatchStateChanged` which only sets `_pendingState`, consumed at the top of the next SimTick. HUD handlers only set dirty flags/labels. No handler mutates sim state synchronously.
...
**Human ownership (⟲ explicit)**: `Human` is a Crowd-owned passive leaf/view — zero upward references, zero subscriptions, driven exclusively by CrowdRoot direct calls. It stays in `Human/Scripts/` for asset cohesion; ownership is documented here and in the final report.
...
`CrowdRoot.SimTick(dt, playerHeading)` phases: ① consume `_pendingState`; ② headings (player passed in; rivals via `RivalAiDriver` plain-C# at 0.4 s cadence counted in sim steps: flee >1.15×, hunt <0.7× when own ≥15, else nearest neutral density; 3-ray wall avoidance) ③ move leaders (CC.Move) ④ steer followers (slots + separation) ⑤ mirror positions → buffer ⑥ grid rebuild ⑦ recruit ⑧ combat ⑨ commit outcomes atomically (sharedMaterial swaps, membership moves between CrowdModels; elimination: IsLeader→false in buffer, ex-leader clone loses CharacterController + shadow casting and joins the eliminator's followers — player leader identically; CameraRoot has already latched a pose snapshot via the elimination event) ⑩ publish coalesced events in ⟲⟲⟲ PINNED ORDER: all `CrowdCountChangedEvent`s first (team id ascending), then `CrowdEliminatedEvent`s — player's elimination FIRST if present, remaining by team id. With GameSession.Finish idempotent, this makes defeat precedence enforceable under synchronous dispatch (a rival's elimination can never produce a premature victory before the player's simultaneous elimination is seen, and standings never use stale counts).
...
3. Validate FBX material (URP Lit + `_BaseColor`); load-or-create the 5 team materials with configured colors.
...
6. Ensure root GO `GameSceneController` + component + serialized refs (config, template=GameArea/Human, camera, cityRoot).
7. `EditorSceneManager.MarkSceneDirty` + save scene + `AssetDatabase.SaveAssets` (only if changed).

`GameSceneValidator.cs` (separate owner): refs non-null; template intact (SMR + Animator on Human_Base + looping clip); ⟲ every `AnimationUtility.GetCurveBindings(clip)` path resolves via `transform.Find` under the Animator; colliders present; material shader assertions; config ranges. Prints `[Validator] PASS/FAIL`; `EditorApplication.Exit` only in batch.
...
| C1 | Crowd/Scripts/CrowdRoot.cs | feature root (SimTick pipeline, spawning, commit, publish) |
...
| H1 | Human/Scripts/Human.cs (rewrite) | passive leaf: Init(material, castShadows), SetTeamMaterial, SetLeader, ApplyPose(pos, headingDeg, speed01), Animator find/offset/speed |
...
| G3 | Game/Scripts/GameplayRoot.cs | root creation, Initialize order, ordered tick driver, Shutdown (reverse order), ReloadRequested |
...
| U1 | Hud/Scripts/HudRoot.cs | runtime uGUI + world labels, tokens |
...
Waves (⟲⟲⟲ E1/E2 in Wave 3 — E1 references GameSceneController): 1 = K1–K6, G6, G7, C3, H1 → gate 1. 2 = T1, G2, C2, G4, G5 → gates 1–2. 3 = C1, G3, G1, U1, E1, E2 → gates 1–3 → gate 4 → cross-review. (All waves' files may be authored in parallel against the pinned contracts; the gates run once after authoring completes.)
...
| Human clones | CrowdRoot creates | CrowdRoot.Shutdown destroys; template only SetActive(false), never destroyed |
| HUD canvas + world leader labels | HudRoot creates | HudRoot.Shutdown destroys |
