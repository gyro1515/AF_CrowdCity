...
{
    public void Initialize();                        // ⟲⟲⟲⟲ config param removed (Batch B): deadzone is a fixed constant, nothing was read from config
...
{
    // ⟲⟲⟲⟲ buildingOccludedMaterial param removed (Batch B): now a [SerializeField] private Material field on the CameraRoot prefab (wired at bake time).
    public void Initialize(UnityEngine.Camera camera, GameConfigSO config, UnityEngine.Transform cityRoot); // clones the serialized buildingOccludedMaterial, enables ShadowCaster on the CameraRoot-owned clone, subscribes both bus events, and registers the 37 generated building colliders
...
{
    public void Initialize(GameConfigSO config, UnityEngine.Transform cityRoot);  // ⟲⟲⟲⟲ humanPrefab param removed; CrowdRoot loads its own spawn target via Resources.Load(HumanResources.HumanPrefab)
    ...
    public void SimTick(float dt);                    // pinned phase order from DESIGN.md §4 (pending-state -> headings -> leader CC.Move -> followers -> mirror -> grid -> recruit -> combat -> atomic commit -> publish). ⟲⟲⟲⟲ Commit on elimination: buffer.IsLeader=false; the ex-leader KEEPS its baked CharacterController (shared prefab CC, never destroyed), only shadow casting is turned off via Human.SetLeader(false), and the clone is routed to the terminal survivor's followers or neutralized (player leader identically). PUBLISH ORDER (pinned): all CrowdCountChangedEvents first (team asc), then CrowdEliminatedEvents — player first if present, remaining by team asc (enforces defeat precedence under synchronous dispatch).
...
```
⟲⟲⟲⟲ SpawnClone = `Instantiate(_humanPrefab, ...)` where `_humanPrefab = Resources.Load<GameObject>(HumanResources.HumanPrefab)` (`"Human/Human"` → `Assets/@Project/Human/Resources/Human/Human.prefab`), owned by CrowdRoot itself (no injected `humanPrefab`; throws if the load returns null). The prefab is created/converged by the editor setup from the fully-configured scene template (Animator on Human_Base child, walk controller, materials) and is active. ⟲⟲⟲⟲ **`Human.cs` and an `enabled` `CharacterController` are BAKED into the shared prefab root** — SpawnClone uses `GetComponent<Human>()`/`GetComponent<CharacterController>()` and never `AddComponent` (throws if either is missing). Publishes authoritative counts/eliminations via cached `EventManager.GetPublisher<CrowdCountChangedEvent>()` / `<CrowdEliminatedEvent>()`. CrowdRoot has no `TMPro` dependency and owns no label, presentation material, or UI lifecycle. ⟲⟲⟲⟲ **All units (leaders, followers, neutrals) carry the same baked enabled CharacterController and move via `CharacterController.Move`** (fixed Y), colliding with city colliders — this is the intended design; unit-vs-unit collision is disabled via a dedicated unit layer + `Physics.IgnoreLayerCollision`. CC spec (radius 0.35, height 1.8, center y 0.9, skinWidth 0.08) is verified against constants by `GameSceneValidator` (never runtime-repaired). City walkable bounds = the `cityRoot/Ground` child renderer bounds shrunk 2 m (same rule as SpawnInitial — Ground REQUIRED). No object pooling: population is conserved.

### G3 `Game/Scripts/GameplayRoot.cs`
```csharp
public sealed class GameplayRoot : UnityEngine.MonoBehaviour
{
    public void Initialize(GameSession session, GameConfigSO config, UnityEngine.Camera mainCamera, UnityEngine.Transform cityRoot);  // ⟲⟲⟲⟲ humanPrefab (2026-07-16), then crowdCountTextStyle + buildingOccludedMaterial (Batch B) params removed — the latter two are now [SerializeField] on the HudRoot / CameraRoot prefabs
    ...
    // Initialize order (PINNED): ⟲⟲⟲⟲ Instantiate all four child-root prefabs via ResourceLoader.LoadRoot<T>() (Resources.Load("Roots/"+typeof(T).Name) + Instantiate + GetComponent — no new GameObject/AddComponent) -> inputRoot.Initialize() -> crowdRoot.Initialize(config, cityRoot) -> cameraRoot.Initialize(mainCamera, config, cityRoot) -> hudRoot.Initialize(session, config, camera)
...
{
    // EXACT serialized field names (editor setup wires them via SerializedObject): config, mainCamera, cityRoot  // ⟲⟲⟲⟲ crowdCountTextStyle + buildingOccludedMaterial removed (Batch B) — moved to the HudRoot / CameraRoot prefabs
    [UnityEngine.SerializeField] private GameConfigSO config;
    // ⟲⟲⟲⟲ crowdCountTextStyle field REMOVED (Batch B): now [SerializeField] on the HudRoot prefab (wired at bake time to Assets/@Project/Hud/CrowdCountTextStyle.asset).
    // ⟲⟲⟲⟲ humanPrefab field REMOVED (2026-07-16): CrowdRoot loads Assets/@Project/Human/Resources/Human/Human.prefab itself via Resources.Load(HumanResources.HumanPrefab).
    ...
    // ⟲⟲⟲⟲ buildingOccludedMaterial field REMOVED (Batch B): now [SerializeField] on the CameraRoot prefab (wired at bake time to Assets/@Project/City/Materials/City_Occluded.mat).
    ...
    // Start: _session = new GameSession(config); ⟲⟲⟲⟲ Instantiate the GameplayRoot prefab (ResourceLoader.LoadRoot<GameplayRoot>()) as a child; _gameplayRoot.Initialize(session, config, mainCamera, cityRoot); _gameplayRoot.ReloadRequested += OnReloadRequested
...
{
    // ⟲⟲⟲⟲ crowdCountTextStyle param removed (Batch B): now a [SerializeField] private TMPTextStyleSO _crowdCountTextStyle field on the HudRoot prefab (wired at bake time).
    public void Initialize(IGameSessionReadOnly session, GameConfigSO config, UnityEngine.Camera worldCamera); // loads the TMP font, creates Canvas + per-team label materials (face/width from the serialized _crowdCountTextStyle), subscribes bus + session.StateChanged
...
`[MenuItem("AF/CrowdCity/Setup Game Scene")] public static void Apply()` — implements DESIGN.md §5 exactly (load-or-create/converge, save only if changed). Constants for all asset paths. Also `public static void ApplyBatch()` (Apply + `EditorApplication.Exit(0/1)`).
⟲⟲⟲⟲ **Human prefab-baking step (new step 4b, after the scene template is fully configured — Animator on Human_Base child, controller, materials — and BEFORE controller wiring):** bake `Human.cs` + an enabled `CharacterController` (pinned spec radius 0.35 / height 1.8 / center (0,0.9,0) / skinWidth 0.08) onto the configured `GameArea/Human` template root, then create/refresh `Assets/@Project/Human/Resources/Human/Human.prefab` (Resources; load key `HumanResources.HumanPrefab` = `"Human/Human"`) via `PrefabUtility.SaveAsPrefabAsset` (load-or-create: overwrite same path each run so it converges), delete the legacy `Assets/@Project/Human/Prefabs/Human.prefab` if present (avoid duplicate merged-Resources keys), and `SetActive(false)` on the scene `GameArea/Human` (no longer used at runtime; runtime clones the prefab), then save the scene. ⟲⟲⟲⟲ **Feature-root + HUD prefab step (step 4c):** load-or-create the logic-only feature-root prefabs `GameplayRoot`/`InputRoot`/`CameraRoot` (`Game/Resources/Roots/`) + `CrowdRoot` (`Crowd/Resources/Roots/`) + `HudRoot` (`Hud/Resources/Roots/`; Canvas + CanvasScaler 1080×1920 match 0.5 + static uGUI timer/leaderboard/hint/result-overlay tree) and the non-root HUD template prefabs `CrowdLabel` + `RivalMarker` (`Hud/Resources/Hud/`); the five root prefabs load via `ResourceLoader.LoadRoot<T>()` (merged-Resources key `Roots/<ClassName>`) and the HUD templates via `HudResources` keys. Load-or-create `Assets/@Project/Hud/CrowdCountTextStyle.asset`. ⟲⟲⟲⟲ (Batch B) wire the single-consumer authored assets onto the consuming feature-root prefabs — `crowdCountTextStyle` → HudRoot prefab, `buildingOccludedMaterial` (`Assets/@Project/City/Materials/City_Occluded.mat`) → CameraRoot prefab — via `PrefabUtility.LoadPrefabContents`/`SaveAsPrefabAsset(out success)`, then reload each saved prefab and fail-fast if the ref does not resolve; wire only `config`, `mainCamera` (scene Main Camera), and `cityRoot` (GameArea/City) onto `GameSceneController` via SerializedObject. ⟲⟲⟲⟲ `GameSceneController.humanPrefab` is no longer wired (field removed; CrowdRoot loads the Human prefab itself via `Resources.Load`). TMP Essential Resources under `Assets/TextMesh Pro/**` are tracked project infrastructure and are not imported or generated by GameSceneSetup. City building generation must preflight source/scene/provenance before writes and converges 37 generated mesh/prefab instances transactionally.

City policy: Full `Setup Game Scene` only read-validates the City source, generated assets, and 37-building hierarchy before other writes; that preflight does not require an existing `GameSceneController` or its references. Full Setup never mutates City assets/hierarchy, but afterward load-or-creates `GameSceneController` and wires `config`, `cityRoot`, and `mainCamera` onto it (⟲⟲⟲⟲ Batch B: `buildingOccludedMaterial` is no longer a GSC ref — the feature-root bake wires it onto the CameraRoot prefab). `[MenuItem("AF/CrowdCity/Setup City Buildings")]` is the sole City convergence entry point, requires exactly one open scene (the active GameScene, no additive scenes), and succeeds even when the controller is absent; ⟲⟲⟲⟲ it only generates/validates the City assets (including `City_Occluded.mat`) and no longer wires the occluded material onto the controller — the CameraRoot-prefab wiring is owned by the feature-root bake (Full Setup / `Bake Feature Root Prefabs`).

### E2 `Game/Editor/GameSceneValidator.cs`
`[MenuItem("AF/CrowdCity/Validate Game Scene")] public static void ValidateMenu()`; `public static bool Validate()` — DESIGN.md §5 assertions incl. AnimationUtility curve-path resolution; logs `[Validator] PASS` / `[Validator] FAIL: <reasons>`; `public static void ValidateBatch()` exits 0/1. ⟲⟲⟲⟲ Validate the PREFAB (`Assets/@Project/Human/Resources/Human/Human.prefab`), not the scene template: it exists, its root carries a baked `Human` component and an enabled `CharacterController` matching the pinned spec (radius 0.35 / height 1.8 / center (0,0.9,0) / skinWidth 0.08 — never runtime-repaired), and it has a `Human_Base` child carrying SkinnedMeshRenderer + Animator whose controller default-state motion is a looping AnimationClip, with every `AnimationUtility.GetCurveBindings(clip)` path resolving via `transform.Find` under the Animator inside the prefab. ⟲⟲⟲⟲ Validate the HUD prefabs (`HudRoot` Canvas/CanvasScaler tree + all HudRoot serialized view refs + inactive ResultOverlay, `CrowdLabel` root TMP, `RivalMarker` Arrow/Count child TMP) and run `ResourcePathValidator` for duplicate merged-Resources load keys, the five root-prefab contracts, and ⟲⟲⟲⟲ (Batch B) the prefab-local moved refs (HudRoot prefab `_crowdCountTextStyle` → CrowdCountTextStyle.asset, CameraRoot prefab `buildingOccludedMaterial` → City_Occluded.mat) — a failure fails the gate. Also assert the `GameSceneController` serialized refs (⟲⟲⟲⟲ Batch B: `config`, `mainCamera`, `cityRoot`) are non-null and point at the intended assets and that the CrowdCountTextStyle asset's `OutlineWidth` is in range; ⟲⟲⟲⟲ `crowdCountTextStyle`/`buildingOccludedMaterial` are no longer GSC refs (validated as prefab-local, above) and `humanPrefab` is no longer asserted (field removed). The required LiberationSans fallback is a HudRoot runtime `Resources.Load` fail-fast dependency, not an Editor-validator assertion.
