...
> **검증 기준 커밋: `a431ff0`** — 이 문서는 이 커밋 시점에 사실임이 확인됐다(verified true as of this commit). `git rev-parse --short HEAD`가 이 해시와 다르면 그 사이 커밋들을 읽기 전까지 이 문서를 신뢰하지 않는다 — 절차는 `CLAUDE.md`/`AGENTS.md`의 "Session start" 규칙.
...
**🔻 문서 전수 감사(2026-07-26) — 종료됐다. 113건 전량 처리됐고, 이 문서 몫은 아래 본문이 정정본이다.**

정본 3문서 + `Perf/MANIFEST.md`를 코드·git·원시 데이터와 전수 대조해 **113건**을 냈다(`PROJECT_MAP` 12 · `WORK_STATE` 49 · `CROWD_GUIDE` 38 · `MANIFEST` 14). in-file 96건은 `58f8b68`("3400 = 렌더-바운드" 주장 철회 포함), 2라운드 교차검증 반영은 `eacd1bb`, 검증 스탬프 메커니즘 1건은 `1fa2fdb`, 문서 간 경계 16건은 `821ab01`(12건 이관·4건 의도적 잔류)과 `a431ff0`(§3 워크플로우 계약 → `CLAUDE.md`/`AGENTS.md`)이 닫았다. 결과 원문 `Docs/Audit/2026-07-26/`는 네 번째 문서가 아니라 한시적 작업 목록이었으므로(`CLAUDE.md` §1.1) 삭제됐다 — `git show ecdfd45 -- Docs/Audit/2026-07-26/`로 읽는다.

**히스토리에서 꺼내 읽는다면 함정 하나: 그 리포트는 단일 에이전트 산출물이라 신뢰 입력이 아니다.** 항목마다 1차 소스(`.cs`/`.asset`/`Perf/` 원시 파일/`git`)로 재검증한 뒤 반영했고, 재검증에서 반증된 지적은 반영하지 않았다 — 2라운드 교차검증에서도 22건 확정·7건 반증이었다. 리포트 산문의 총계도 자기 표와 어긋난다(표가 정본).

**경계 기준으로는 이 문서 소관이 아니지만 의도적으로 남긴 4건**(목적지가 없거나 옮기면 사실이 사라진다 — 다시 옮기려 들지 말 것):
