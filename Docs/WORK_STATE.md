...
> **Verified-against commit: `ceb7e31`** — this document was confirmed true as of that commit. If `git rev-parse --short HEAD` differs from that hash, do not trust this document until you have read the commits in between — the procedure is the "Session start" rule in `CLAUDE.md`/`AGENTS.md`.
...
| Combat direction and budget are computed **only from the tick-start counts (`_startCounts`)** | snapshot `CombatResolver.cs:242-244`·`:264` → consumed at `:356`, `:364` (dynamic rate), `:395`, `:424`, `:555`. Pair enumeration is the fixed nested loops `:339-341`, `:420-422` | **Permutation invariance of the pair-enumeration order comes from this rule alone.** The enumeration loops are fixed, so there is no room to build a permutation, and what the dedicated test actually shuffles is only agent insertion order (`CombatResolverTests.cs:318`, dynamic-rate twin `:1280`) → use mid-tick adjusted counts and **the tests still pass while this invariance silently disappears** (rationale in `CrowdCity/CROWD_GUIDE.md` §9) |
...
- **The dynamic combat conversion rate (`SimTuning.UseDynamicConvertRate`) ships OFF, and the oracle gate above only means something while it stays OFF.** ON, the per-pair rate becomes `_startCounts[winner] * ConvertPerSecondPerMember` instead of the fixed `ConvertPerSecond` (`CombatResolver.cs:364`; fields `SimTuning.cs:74`·`:80`, clamp `GameConfigSO.cs:283`, validator `GameSceneValidator.cs:730`). The shipping asset authors the toggle `0` (`GameConfig.asset:47`, multiplier `:48`), so the shipping path evaluates exactly the old expression and the oracle md5 above reproduces unchanged. Three things to know before flipping it: ① it **changes the simulation result**, so that md5 is void and must be re-baselined in the same commit that flips it; ② the winner is by construction always the strictly larger team (`CombatResolver.cs:356`), so a count-scaled rate **snowballs** — that is the intended design, not a defect; ③ it is **completely inert while `RateLimitConversion` is OFF**, because that path skips the budget gate and converts every contacting victim regardless of rate (`CombatResolver.cs:432`). Coverage: `CombatResolverTests.cs:1145`, `:1202`, `:1254`, `:1280` (each poisons `ConvertPerSecond` with an absurd value so taking the wrong branch fails loudly).
...
| `CombatResolver` team-ID / `MatchRules` array-length defensive validation | `CombatResolver.cs:261-265` (lower bound only) · `MatchRules.cs:54-56` (no length-consistency check) | Only internal callers exist — an **unreachable scenario** (CLAUDE.md "No error handling for impossible scenarios") |
