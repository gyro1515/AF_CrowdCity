...
{
    // 최대 인원 = 리더 4 + 중립 150 = 154. 전원이 한 crowd에 합류해도 재할당이 없도록 여유를 둔다.
    private const int FollowerCapacity = 160;
    ...
    /// <summary>
    /// 팀 id, 팀 material, leader clone, leader의 AgentBuffer index로 model을 생성한다.
    /// follower 목록은 최대 인원에 맞춰 미리 할당한다.
    /// </summary>
    public CrowdModel(int teamId, Material teamMaterial, Human leader, int leaderAgentIndex)
    {
        TeamId = teamId;
